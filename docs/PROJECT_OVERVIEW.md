...
При `ENABLE_USER_AUTH=true` проекты и jobs получают `owner_id`, а project/job/file endpoints проверяют `Authorization: Bearer <access_token>`. Это уже закрывает базовую multi-user изоляцию в MVP, но production всё ещё требует managed auth/OIDC, organizations, roles, audit logs и session management UI.
...
2. `VisualPlanService` формирует production-план визуалов.
   - Для каждой сцены задаёт `visual_type`, `avatar_visible`, `source_query`, `visual_prompt` и причину выбора.
   - Для `ai_news_avatar` раскладывает ролик на хук, fullscreen avatar, PIP, screen demo, AI b-roll, вывод и CTA.
   - Пишет `visual_plan.json`.

3. `SourceService` собирает источники.
   - Берёт URL, которые указал пользователь.
   - Использует per-scene `source_query` для поиска и матчинг источников на `screenshot`/`screen_demo`.
   - Добавляет curated official websites по ключевым словам темы.
   - Блокирует localhost/private/plain HTTP по умолчанию.

4. `ScreenshotService` создаёт визуальные референсы.
   - По умолчанию рисует offline fallback-карточки.
   - При `ENABLE_BROWSER_SCREENSHOTS=true` пробует Playwright screenshots.

5. `VoiceService` создаёт audio.
   - По умолчанию пишет WAV-заглушки.
   - При `voice_provider=openai` пробует OpenAI TTS.
   - Пишет `voice_manifest.json`, SRT и VTT.

6. `VisualService` рисует PNG-визуалы.
   - Поддерживает `ai_slide`, `screenshot`, `table`, `diagram`, `avatar_fullscreen`, `avatar_pip`, `screen_demo`, `ai_broll`, `big_caption`, `cta`.
   - Использует Pillow.
   - При `ENABLE_MODEL_IMAGES=true` пробует OpenAI image generation для `ai_broll`.
   - Пишет `render_templates.json` и `visual_assets_manifest.json` с composition layers / replacement slots.

7. `AvatarService` готовит avatar-сцены.
   - Без ключей пишет fallback `avatar_manifest.json`.
   - При `HEYGEN_API_KEY` + `HEYGEN_AVATAR_ID` отправляет avatar-сцены в HeyGen `/v3/videos`.
   - Сохраняет `video_id`, статус, URL и локальный MP4, если провайдер вернул готовый файл.

8. `RenderService` собирает MP4.
   - Использует FFmpeg.
   - Если у сцены есть локальный `avatar_video_path`, собирает per-scene avatar composite: fullscreen или PIP поверх PNG-визуала.
   - Пишет `production_timeline` в `render_manifest.json`.
...
- project duplicate/delete controls;
- scene editor for title/narration/duration, insert/delete/reorder, and one-slide regeneration;
...
- YouTube metadata/export package.
- API key protection.
- Optional file-backed user auth через `ENABLE_USER_AUTH`, bearer sessions and project/job ownership checks.
...
- Есть MVP user accounts/project ownership, но нет production-grade OIDC, roles, organizations, email verification, audit logs и session management UI.
- Нет billing/limits.
